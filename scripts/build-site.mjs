...
const frame = (title, description, body, pathname = "/", structured = "") => `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${esc(`${config.url.replace(/\/$/, "")}${pathname}`)}"><link rel="stylesheet" href="/rakuten-blogtool/style.css">${structured}</head><body><header><a href="/rakuten-blogtool/"><strong>${esc(config.name)}</strong></a><span>商品選びをシンプルに</span></header><main>${body}</main><footer><p>${esc(config.description)}</p><p><a href="/rakuten-blogtool/about.html">運営方針・広告について</a></p></footer></body></html>`;
...
for (const p of posts) {
  ...
  await fs.writeFile(path.join(out, "posts", `${p.slug}.html`), frame(`${p.title} | ${config.name}`, p.description, body, `/posts/${p.slug}.html`, `<script type="application/ld+json">${data}</script>`));
}
